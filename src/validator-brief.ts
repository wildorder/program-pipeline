...
workstream roster, dependencies, and scope; the program document references
them by id. Flag a program-document restatement that contradicts the manifest
as a blocker, and a merely duplicated restatement as a minor duplication
finding. Where repetition is legitimate (architecture prose referencing an
interface, workstreams[].scope entries), checkedSubjects includes every such
copy plus the runtime implementations. The scope copy is mandatory because it
may be the only copy the workstream author receives. Resolve conditional
members against the repository before accepting any set declared equal.
